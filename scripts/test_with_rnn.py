...
class HumanFollowTrain:
    ...
    def main(self):
        ...
        # Model Description
        model.add(Conv2D(32, kernel_size=9, strides=(2,2), activation='relu', input_shape=(128,128,1)))
        model.add(BatchNormalization())
        model.add(Conv2D(16, kernel_size=5, strides=(2,2), activation='relu'))
        model.add(Conv2D(8, kernel_size=3, strides=(2,2), activation='relu'))
        model.add(Flatten())
        model.add(Reshape((1,1352)))
        model.add(LSTM(30))
        model.add(Dense(2))
